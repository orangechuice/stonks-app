      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
            ...
            {/* Mini Indices Bar (e.g. S&P 500, NASDAQ) */}
            {indexQuotes.length > 0 && (
              ...
              >
                {indexQuotes.slice(0, 3).map((idxQuote) => {
                  const shade = getColorShade(idxQuote.regularMarketChangePercent);
                  const isSelected = idxQuote.symbol.toUpperCase() === symbol.toUpperCase();
                  return (
                    <button
                      key={idxQuote.symbol}
                      onClick={(e) => {
                        e.stopPropagation();
                        if (onSelectSymbol) onSelectSymbol(idxQuote.symbol);
                      }}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    >
                      <span>{idxQuote.shortName || idxQuote.symbol}</span>
                      <span style={{ color: shade.textColor }}>
                        {formatPercent(idxQuote.regularMarketChangePercent)}
                      </span>
                    </button>
                  );
                })}
